...
class ProteinBertModel(nn.Module):
    ...
    def predict_contacts(self, tokens):
        return self(tokens, return_contacts=False)["contacts"]
...
class MSATransformer(nn.Module):
    ...
    def forward(self, tokens, repr_layers=[], need_head_weights=True, return_contacts=False):
        ...
        batch_size, num_alignments, seqlen = tokens.size()  # torch.Size([1, 64, 126])
        padding_mask = tokens.eq(self.padding_idx)  # B, R, C
        if not padding_mask.any():
            padding_mask = None

        x = self.embed_tokens(tokens)  # (1,64,126,768)

        x += self.embed_positions(tokens.view(batch_size * num_alignments, seqlen)).view(x.size())  # (1,64,126,768)
        ...
        if self.msa_position_embedding is not None:
            ...
            x += self.msa_position_embedding[:, :num_alignments]  # (1,64,126,768)

        x = self.emb_layer_norm_before(x)  # (1,64,126,768)

        x = self.dropout_module(x)  # (1,64,126,768)
        ...
        x = x.permute(1, 2, 0, 3)  # (64,126,1,768)
        ...
        x = self.emb_layer_norm_after(x)  # (64,126,1,768)
        x = x.permute(2, 0, 1, 3)  # R x C x B x D -> B x R x C x D [1, 64, 126, 768]
        if self.prepend_bos:
            x = x[..., 1:, :]
        ...
        if need_head_weights:
            ...
            # row_attentions = row_attentions.view(1, -1, row_attentions.size(3), row_attentions.size(4))
            if self.prepend_bos:
                row_attentions = row_attentions[..., 1:, 1:]
            batch_size, layers, heads, seqlen, _ = row_attentions.size()
            row_attentions = row_attentions.view(batch_size, layers * heads, seqlen, seqlen)
    ...
    def predict_contacts(self, tokens):
        return self(tokens, return_contacts=False)["logits"], self(tokens, return_contacts=False)["row_attentions"]
