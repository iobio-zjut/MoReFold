...
init(extra_options = "-constant_seed -mute all -read_only_ATOM_entries -ignore_zero_occupancy false")
...
def extract_EnergyDistM(pose, energy_terms):
    ...
    Returns:
        output: 3D matrix of size feature by protein length by protein length. (tbt)
...
def extract_Bfactor(pose):
    bfactors = []
    chain1_start_pose_id = pose.chain_begin(1)  # 返回某链起始的rosetta index
    chain1_end_pose_id = pose.chain_end(1)
    for index in range(chain1_start_pose_id, chain1_end_pose_id + 1):
        bfactors.append(pose.pdb_info().bfactor(res=index, atom_index=2))  # 一般第二个原子是CA

    bfactors_array = np.array(bfactors)
    mean = np.mean(bfactors_array)
    std = np.std(bfactors_array)
    for i in range(len(bfactors)):
        bfactors[i] = (bfactors[i] - mean) / (std+1e-10)

    bfactors = np.array(bfactors)
    # 非晶体蛋白的b-factor为0
    if True in np.isnan(bfactors):
        raise Exception("存在nan值")
    return np.expand_dims(bfactors, axis=0)
...
def process(args):
    ...
    try:
        ...
        usr = extract_USR(pose)  # 加入usr
        
        try:
            bfactors = extract_Bfactor(pose)
        except:
            print('bfactors 特征处理失败')
            with open("bfactors_error.txt", 'a')as fff:
                fff.write(filename+'\n')
            bfactors = np.zeros((1,len(pose)))
        prop = np.vstack((prop, usr, bfactors))
