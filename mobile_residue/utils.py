...
def getData(tmp, msa_path, cutoff=0, use_msa_transformer=False, use_bfactor=False):
    data = np.load(tmp)
    if use_msa_transformer:
    ...
    if not use_bfactor and prop.shape[-1]==56:
        prop = prop[:, :-1]
